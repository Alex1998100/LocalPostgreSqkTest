...
fs.readFile("./root2.json", async (err, data)=> {
    //await Select(db, "truncate table public.entry;", selectCallback);
    ...
    for (let i = 0; i < obj.length; i++) {
        await Insert(db,`INSERT INTO public.entry(parent, id, kind, name, mime) VALUES ('1VF7Nbcs1FikmSYuFbf5FqII2ZIeFK5ef', '${obj[i].id}', '${obj[i].kind}', '${obj[i].name}','${obj[i].mimeType}');`,insertCallback)
    }
